...
`source_type` sólo admite `drive` por ahora. `max_files` existe para diagnóstico: una
...
2. Enumera recursivamente la carpeta de la fuente externa y obtiene metadata. La URI
   persistida tiene forma `drive://file/{drive_file_id}`, por ejemplo.
