...
from common.models import QUEUE_PDF
...
from pdf.worker import publish_pending_outbox, run_pdf_worker
from staging.adapters import PostgresTextExtractionRepository


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Consume Queue-PDF messages and stage PDF text chunks."
    )
    parser.add_argument(
        "--source-queue-name",
        default=QUEUE_PDF,
        help=f"RabbitMQ source queue name. Default: {QUEUE_PDF}",
    )
    parser.add_argument(
        "--env-file",
        default=None,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Publish pending Queue-OCR-Urgente/Queue-OCR/Queue-Entity "
            "outbox messages and exit."
        ),
    )
    parser.add_argument(
        "--max-messages",
        type=int,
        default=None,
        help="Process at most this many Queue-PDF messages.",
    )
    parser.add_argument(
        "--dev-mode",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    max_pdf_retries = optional_int_env("PDF_MAX_RETRIES", 3)
    quarantine_log_path = os.environ.get("PDF_QUARANTINE_LOG_PATH")
