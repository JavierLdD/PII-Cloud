...
from pdf.worker import OCR_POLICY_POISON, process_pdf_payload  # noqa: E402
from staging.adapters import PostgresTextExtractionRepository  # noqa: E402


LOGGER = logging.getLogger("cloud_text_pdf_extract_job")
...
def main() -> int:
    ...
    with PostgresTextExtractionRepository(config.database_url) as repository:
        with PubSubOutboxRepository(config.database_url) as outbox_repository:
            ...
            processed = drain_subscription(
                config=config,
                puller=puller,
                handle_message=lambda message: handle_pdf_message(
                    message=message,
                    config=config,
    ...
    LOGGER.info("pdf_extract_job_finished processed=%s", processed)
    return 0


def handle_pdf_message(
    *,
    ...
    try:
        RoutedFileMessage.from_payload(message.payload)
        ...
        _publish_direct_poison(
            ...
            payload=build_poison_payload_from_message(
                payload=message.payload,
                stage="pdf",
                reason="unsupported_message_for_pdf_job",
                error=str(exc),
            ),
        )
        return
    ...
    if result.status == TEXT_EXTRACTION_COMPLETED_STATUS:
        ...
        LOGGER.info(
            "processed_pdf file_id=%s status=%s chunks=%s published=%s",
    ...
    poison_payload = build_poison_payload_from_stored_file(
        ...
        stage="pdf",
        reason=_reason_for_pdf_result(result),
    ...
    LOGGER.info(
        "poisoned_pdf file_id=%s status=%s error=%s",
        result.message.file_id,
        result.status,
        result.error,
    )
...
def _persist_pdf_failure(
    *,
    ...
    routed = RoutedFileMessage.from_payload(payload)
...
def _reason_for_pdf_result(result: PdfProcessingResult) -> str:
    if result.error == "pdf_requires_ocr_but_ocr_is_disabled":
        return "ocr_required"
    error_reason = _reason_from_error(result.error)
    if error_reason:
        return error_reason
    if any(page.reason == "ocr_required_but_disabled" for page in result.pages):
        return "ocr_required"
    if any(page.status == PAGE_FAILED_STATUS for page in result.pages):
