...
JOB_NAME="${JOB_NAME:-text-pdf-extract-job}"
TASKS="${TASKS:-1}"
...
CPU="${CPU:-1}"
MEMORY="${MEMORY:-1Gi}"
ENV_VARS_FILE="${ENV_VARS_FILE:-${PROJECT_ROOT}/Cloud/Text-PDF-Extract-Job/config/env.sample.yaml}"
...
cmd=(
  ...
  --cpu "${CPU}"
  --memory "${MEMORY}"
  --labels "app=pii,component=text-pdf-extract"
)
